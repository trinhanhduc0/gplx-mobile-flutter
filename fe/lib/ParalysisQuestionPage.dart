...
class ParalysisQuestionState extends State<ParalysisQuestion> {
  int _total = 0;
  int _received = 0;
  ...
  @override
  void dispose() {
    // Cancel any ongoing timers or animations here
    super.dispose();
  }
  ...
  Future<List<Cau>> loadFileQuestions(int id) async {
    ...
    if (await AppUtil.fileExist(path: filePath)) {
      ...
      List<Cau> dscau = jsonResponse
          .map((e) => Cau.fromJson(e))
          .where((e) => e.ttcaus.diemliet)
          .toList();
      print(dscau.length);
      return dscau;
    } else {
  ...
  Future<void> fetchQuestion() async {
    ...
    _response.stream.listen((value) {
      ...
      setState(() {
        ...
        choosequestion = List<int>.filled(questions.length, -1);
        prefs.setString("CHOOSES", jsonEncode(choosequestion));
        questions = questions.where((e) => e.ttcaus.diemliet == true).toList();
      });
    });
  }
...
class ParalysisQuestion extends StatefulWidget {
  const ParalysisQuestion({super.key});

  @override
  ParalysisQuestionState createState() => ParalysisQuestionState();
}
